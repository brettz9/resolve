...
module.exports = function resolveSync(x, options) {
    ...
    var mainFields = opts.mainFields || ['main'];
    ...
    function loadAsDirectorySync(x) {
        ...
        if (isFile(pkgfile)) {
            ...
            if (pkg) {
                var prop = mainFields.find(function (mainField) {
                    return pkg[mainField];
                });
                if (prop) {
                    if (typeof pkg[prop] !== 'string') {
                        var mainError = new TypeError('package “' + pkg.name + '” `' + prop + '` must be a string');
                        mainError.code = 'INVALID_PACKAGE_MAIN';
                        throw mainError;
                    }
                    if (pkg[prop] === '.' || pkg[prop] === './') {
                        pkg[prop] = 'index';
                    }
                    try {
                        var m = loadAsFileSync(path.resolve(x, pkg[prop]));
                        // eslint-disable-next-line max-depth
                        if (m) return m;
                        var n = loadAsDirectorySync(path.resolve(x, pkg[prop]));
                        // eslint-disable-next-line max-depth
                        if (n) return n;
                    } catch (e) {}
                }
